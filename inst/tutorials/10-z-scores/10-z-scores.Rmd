...
dllink <- download_link(
  link = "https://dwillis.github.io/sports-data-files/volleyball1822.csv",
...
seasons <- read_csv("https://dwillis.github.io/sports-data-files/volleyball1822.csv")
...
The first thing we're going to do is deal with having multiple seasons. We want the z-score to be centered on the average for *that season*, so each school gets ranked against their competition, not all teams in all seasons. That way, we compare how good the 2018 Terps were compared to 2018 teams, vs 2022 teams, which is impossible. A team from 2018 can't play a team from 2022. How do we make z-scores for each season? Simple: group by.

Then, we're *NOT* going to use summarize. We'll use mutate. That has the effect of putting all the 2022 teams together in a pile, calculating each teams z-score and then just leaving it there instead of folding it all together into a single number (like summarize would).
...
So now we have a dataframe called `teamzscore` that has 1669 volleyball teams with Z scores. What does it look like?
