...
First, we need data from the 2022 college football season. **For purposes of this exercise, you don't need to do this. The data is included here if you want to try this in your own notebook.**
...
So on a scale of -1 to 1, where 0 means there's no relationship at all and 1 or -1 means a perfect relationship, penalty yards and whether or not the team scores more points than it give up are at 0.531. You could say they're 53 percent related in the positive -- sometimes more rushing yards means more points, sometimes there's not. It's about the worst outcome for an argument -- it's 50-50. Run The Damn Ball Guy's attitude is unearned.
...
2. Second, we look at the Adjusted R-squared value. It's right above the p-value. Adjusted R-squared is a measure of how much of the difference between a teams point spread can be explained by rushing yards. Our correlation coefficient said they're 50 percent related to each other, but rushing yard's ability to explain the difference between point spreads in games? About 26 percent. That means we can explain 26 percent of the difference in score just by looking at a team's rushing yards. That's ... not super conclusive. It's not nothing, but it's not the clattering proof Run The Damn Ball Guy was looking for. Again, we should quit and do literally anything with our life other than talk to Run The Damn Ball Guy.
3. The third thing we can look at, and we only bother if the first two are meaningful, is the coefficients. In the middle, you can see the (Intercept) is -19.700066 and the RushingYds coefficient is 0.139308 Remember high school algebra? Remember learning the equation of a line? Remember swearing that learning `y=mx+b` is stupid because you'll never need it again? Surprise. It's useful again. In this case, we could try to predict a team's score differential in a game -- will they score more than they give up -- by using `y=mx+b`. In this case, y is the aggregate score, m is 0.139308 and b is -19.700066 So we would multiply a teams total rushing yards by 0.139308 and then subtract 19.700066 to it. The result would tell you what the total aggregate score in the game would be, according to our model. Chance that your even close with this? About 26 percent. 

What does that mean? Let's use a simple example. Let's say you rush for 100 yards. 100 * 0.139308 is roughly 14 points. So 100 yards of rushing gets you 14 points ... until you factor in the intercept, which is -19 points. Meaning, according to our model, you rush for 100 yards, you lose by about a touchdown. 
...
Second, Adjusted R-squared: 0.08558. With rushing yards, we could predict 25 percent of the variation between score margin by looking at a team's rushing yards. But with passing? A whopping 8 percent.

Ohhh no. What have we done?

Third, the coefficients: In this case, our `y=mx+b` formula looks like `y = 0.069921x - 13.998850`. So if we were applying this, if our team passes for 250 yards, what happens?.  

```{r calculate, exercise=TRUE}
0.069921*250 - 13.998850
```
