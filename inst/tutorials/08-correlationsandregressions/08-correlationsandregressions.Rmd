...
First, we need data from the 2024 college football season. **For purposes of this exercise, you don't need to do this. The data is included here if you want to try this in your own notebook.**
...
correlations <- read_csv("https://dwillis.github.io/sports-data-files/footballlogs24.csv")
...
So on a scale of -1 to 1, where 0 means there's no relationship at all and 1 or -1 means a perfect relationship, rushing yards and whether or not the team scores more points than it give up are at 0.603. You could say they're 60 percent related in the positive -- sometimes more rushing yards means more points, sometimes there's not. It's not the worst outcome for an argument, but it's not a lot greater than 50-50. Run The Damn Ball Guy's attitude is unearned.
...
2. Second, we look at the Adjusted R-squared value. It's right above the p-value. Adjusted R-squared is a measure of how much of the difference between a teams point spread can be explained by rushing yards. Our correlation coefficient said they're 50 percent related to each other, but rushing yard's ability to explain the difference between point spreads in games? About 31 percent. That means we can explain 31 percent of the difference in score just by looking at a team's rushing yards. That's ... not super conclusive. It's not nothing, but it's not the clattering proof Run The Damn Ball Guy was looking for. Again, we should quit and do literally anything with our life other than talk to Run The Damn Ball Guy.
3. The third thing we can look at, and we only bother if the first two are meaningful, is the coefficients. In the middle, you can see the (Intercept) is -22.07112 and the RushingYds coefficient is 0.15490. Remember high school algebra? Remember learning the equation of a line? Remember swearing that learning `y=mx+b` is stupid because you'll never need it again? Surprise. It's useful again. In this case, we could try to predict a team's score differential in a game -- will they score more than they give up -- by using `y=mx+b`. In this case, y is the aggregate score, m is 0.15490 and b is -22.07112. So we would multiply a teams total rushing yards by 0.15490 and then subtract 22.07112 from it. The result would tell you what the total aggregate score in the game would be, according to our model. Chance that your even close with this? About 31 percent. 

What does that mean? Let's use a simple example. Let's say you rush for 100 yards. 100 * 0.15490 is roughly 15 points. So 100 yards of rushing gets you 15 points ... until you factor in the intercept, which is -22 points. Meaning, according to our model, you rush for 100 yards, you lose by about a touchdown. 
...
A charitable reading of this would be: yes, there's a relationship here, but there's also a lot of variability. 

## But what about passing the ball?

So we've **firmly** established that Run The Damn Ball Guy arguments are not great. But is the Playstation Offense the answer? 
...
Second, Adjusted R-squared: 0.08387. With rushing yards, we could predict 31 percent of the variation between score margin by looking at a team's rushing yards. But with passing? A whopping 8 percent.

Ohhh no. What have we done?

Third, the coefficients: In this case, our `y=mx+b` formula looks like `y = 0.075471x - 14.546726`. So if we were applying this, if our team passes for 250 yards, what happens?.  

```{r calculate, exercise=TRUE}
0.075471*250 - 14.546726
```
...
By our model, a team passing for 250 yards wins by 4 points. Chances of that being right? About 8 percent. Not good at all.
