...
players <- read_csv("https://dwillis.github.io/sports-data-files/ncaa_mens_lacrosse_playerstats_2023.csv") %>% clean_names()
...
clusters <- kmeans(playersscaled, centers = 8, nstart = 25)
```
...
Interpreting this output, the very first thing you need to know is that **the cluster numbers are meaningless**. They aren't ranks. They aren't anything. After you have taken that on board, look at the cluster sizes at the top. Clusters 1, 3 and 6 are pretty large compared to others. That's notable. Then we can look at the cluster means. For reference, 0 is going to be average. So group 4 is above average on goals. Group 3 is below, group 6 is slightly above. 
...
Now we have a dataframe called playercluster that has our player names and what cluster they are in. The fastest way to find Joey Spallina is to double click on the playercluster table in the environment and use the search in the top right of the table. Because this is based on some random selections of points to start the groupings, these may change from person to person, but Spallina is in Group 2 in my data. 
...
So Joey is in cluster 2 in my data, which if you look at our clusters, puts him in the cluster with all above average metrics. What does that look like? Let's look at scoring by creating a scatterplot using goals on the x-axis and assists on the y-axis, adding Spallina in orange so he'll stand out.
...
So there are the 14 attackmen most like Joey Spallina. Pretty good company, and also a good starting point for making comparisons.