...
```
volleyballplayers <- read_csv("~/SportsData/ncaa_volleyball_playermatchstats_2024.csv")
```
...
The `read_csv` bits are pretty obvious, except for one thing. What happens in the quote marks is the path to the data. In there, I have to tell R where it will find the data. The easiest thing to do when you are working on your own computer, if you are confused about how to find your data, is to put your data in the same folder as as your notebook (you'll have to save that notebook first). If you do that, then you just need to put the name of the file in there (ncaa_womens_volleyball_playerstats_2023.csv). For this exercise, we're pulling the data from a website, and the path that we'll enter is the url of the data.

Your first task is to import the data. For this exercise, you need to simply run this:

```{r load-data, message=FALSE, warning=FALSE}
volleyballplayers <- read_csv("https://dwillis.github.io/sports-data-files/ncaa_volleyball_playermatchstats_2024.csv")
...
volleyballplayers |>
  group_by(school_name, player_position) |>
  summarise(
    total = n()
  ) |> 
  arrange(desc(total))
...
glue("The kill leader in the NCAA? {killsleader$player_full_name} of {killsleader$school_name} with {killsleader$kills} kills. The lowest? A whole lot of people with 0.")
```
