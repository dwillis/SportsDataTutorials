...
Let's go back to our net yards model from the previous lesson. We need data from the 2024 college football season. **For purposes of this exercise, you don't need to do this. The data is included here if you want to try this in your own notebook.**
...
We've seen this output before, but let's review because if you are using scatterplots to make a point, you should do this. First, note the Min and Max residual at the top. A team has underperformed the model by 43 points (!), and a team has overperformed it by 42 points (!). The median residual, where half are above and half are below, is just slightly below the fit line. Close here is good. 
...
Looking at this table, what you see here are the teams who scored more than their net yards would indicate. Those are some interesting games.
...
* Our min and max go from -73 to positive 74
* Our adjusted R-squared is ... -0.0007425. Not much at all. 
* Our p-value is ... 0.9527, which is more than than .05. 
...
First, note all of the biggest misses here are all blowout games. The worst games of the 2024 season, the worst being South Alabama vs Northwestern State. The model missed that differential by ... 74 points. The margin of victory? 77 points. In other words, this model is terrible. But let's look at it anyway. 
